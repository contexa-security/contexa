...
/**
 * One prompt benchmark round executed through the real WebClient + MFA + protected-resource path.
 *
 * This is the concrete "when" contract for the sandbox full-stack benchmark.
 * Each round can change request path, client IP, browser fingerprint, device alias,
 * dwell/cadence, and semantic expectation while keeping the same authenticated account.
 */
public record SandboxPromptRoundPlan(
        String roundKey,
        String requestPath,
        String clientIp,
    ...
    public boolean startsNewSession() {
        return sessionMode == SandboxPromptSessionMode.NEW_SESSION;
    }
}
