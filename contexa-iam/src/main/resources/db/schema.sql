...
-- ----------------------------------------------------------------------------
-- Source migration: V20260707_01__official_llm_decision_test_model.sql
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS official_llm_decision_test_case (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    case_id VARCHAR(128) NOT NULL UNIQUE,
    case_name VARCHAR(255) NOT NULL,
    case_family VARCHAR(128) NOT NULL,
    definition_version VARCHAR(64) NOT NULL,
    expected_action VARCHAR(32) NOT NULL,
    acceptable_actions VARCHAR(255) NOT NULL,
    required_evidence_refs TEXT NOT NULL,
    forbidden_evidence_refs TEXT NOT NULL,
    required_reasoning_claims TEXT NOT NULL,
    forbidden_reasoning_claims TEXT NOT NULL,
    covered_metric_codes VARCHAR(255) NOT NULL,
    explanation TEXT NOT NULL,
    critical BOOLEAN NOT NULL DEFAULT TRUE,
    blocks_operational_pass BOOLEAN NOT NULL DEFAULT TRUE,
    active BOOLEAN NOT NULL DEFAULT TRUE,
    created_at TIMESTAMP(6) WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP(6) WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_official_llm_decision_test_case_family
    ON official_llm_decision_test_case(case_family, active);

CREATE TABLE IF NOT EXISTS official_llm_decision_test_result (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    result_id VARCHAR(160) NOT NULL UNIQUE,
    package_id VARCHAR(256),
    aggregate_run_id VARCHAR(256),
    run_id VARCHAR(256),
    request_id VARCHAR(256),
    ...
    blocked_operational_pass BOOLEAN NOT NULL DEFAULT FALSE,
    failure_category VARCHAR(80),
    failure_reason TEXT,
    evidence_match_status VARCHAR(80),
    reasoning_match_status VARCHAR(80),
    evaluated_at TIMESTAMP(6) WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    created_at TIMESTAMP(6) WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_official_llm_decision_test_result_package
    ON official_llm_decision_test_result(package_id, aggregate_run_id, metric_code, case_id);

CREATE INDEX IF NOT EXISTS idx_official_llm_decision_test_result_request
    ON official_llm_decision_test_result(request_id, run_id);

CREATE INDEX IF NOT EXISTS idx_official_llm_decision_test_result_case
    ON official_llm_decision_test_result(case_id, evaluated_at DESC);

CREATE TABLE IF NOT EXISTS official_llm_decision_mutation_result (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    mutation_result_id VARCHAR(160) NOT NULL UNIQUE,
    package_id VARCHAR(256),
    aggregate_run_id VARCHAR(256),
    base_run_id VARCHAR(256),
    mutated_run_id VARCHAR(256),
    request_id VARCHAR(256),
    metric_code VARCHAR(32) NOT NULL,
    case_id VARCHAR(128) NOT NULL,
    mutation_key VARCHAR(160) NOT NULL,
    base_action VARCHAR(32),
    mutated_action VARCHAR(32),
    expected_action_changed BOOLEAN NOT NULL,
    actual_action_changed BOOLEAN NOT NULL,
    passed BOOLEAN NOT NULL,
    failure_reason TEXT,
    evaluated_at TIMESTAMP(6) WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    created_at TIMESTAMP(6) WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP
...
CREATE INDEX IF NOT EXISTS idx_official_llm_decision_mutation_package
    ON official_llm_decision_mutation_result(package_id, aggregate_run_id, case_id);

CREATE INDEX IF NOT EXISTS idx_official_llm_decision_mutation_runs
    ON official_llm_decision_mutation_result(base_run_id, mutated_run_id);

CREATE TABLE IF NOT EXISTS official_llm_decision_replay_result (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    replay_result_id VARCHAR(160) NOT NULL UNIQUE,
    package_id VARCHAR(256),
    aggregate_run_id VARCHAR(256),
    first_run_id VARCHAR(256),
    replay_run_id VARCHAR(256),
    request_id VARCHAR(256),
    metric_code VARCHAR(32) NOT NULL,
    case_id VARCHAR(128) NOT NULL,
    expected_stable_action VARCHAR(32) NOT NULL,
    first_action VARCHAR(32),
    replay_action VARCHAR(32),
    passed BOOLEAN NOT NULL,
    failure_reason TEXT,
    evaluated_at TIMESTAMP(6) WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    created_at TIMESTAMP(6) WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP
...
CREATE INDEX IF NOT EXISTS idx_official_llm_decision_replay_package
    ON official_llm_decision_replay_result(package_id, aggregate_run_id, case_id);

CREATE INDEX IF NOT EXISTS idx_official_llm_decision_replay_runs
    ON official_llm_decision_replay_result(first_run_id, replay_run_id);
