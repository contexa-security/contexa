...
INSERT INTO official_verification_metric_definition (
    metric_code, definition_version, metric_name, metric_group, purpose, evidence_contract, blocking_scope
)
SELECT metric_code, definition_version, metric_name, metric_group, purpose, evidence_contract, blocking_scope
  FROM (
      VALUES
          ('EIR', '2026.05.01-pqa12-p1', 'Evidence Integrity and Representativeness', 'sealed-evidence',
          ...
          ('CCR', '2026.05.01-pqa12-p1', 'Context Completeness Rate', 'context-completeness',
          ...
          ('CCSR', '2026.05.01-pqa12-p1', 'Context Consistency Rate', 'context-consistency',
          ...
          ('PFR', '2026.05.01-pqa12-p1', 'Prompt Fidelity Rate', 'prompt-fidelity',
          ...
          ('MTR', '2026.05.01-pqa12-p1', 'Metric Traceability and Reproducibility', 'traceability',
          ...
          ('CoR', '2026.05.01-pqa12-p1', 'Context Contamination Rate', 'context-contamination',
          ...
          ('RAP', '2026.05.01-pqa12-p1', 'RAG Authorization Precision', 'rag-authorization',
          ...
          ('RPI', '2026.05.01-pqa12-p1', 'Round Prompt Invariance', 'round-consistency',
          ...
          ('BMA', '2026.05.01-pqa12-p1', 'Baseline Maturity Accuracy', 'learning-baseline',
          ...
          ('USNS', '2026.05.01-pqa12-p1', 'User-Specific Novelty Sensitivity', 'user-novelty',
          ...
          ('BSR', '2026.05.01-pqa12-p1', 'Behavioral Surprise Resolution', 'behavior-context',
          ...
          ('PRE', '2026.05.01-pqa12-p1', 'Protectable Resource Eligibility', 'resource-eligibility',
  ...
  ) AS v(metric_code, definition_version, metric_name, metric_group, purpose, evidence_contract, blocking_scope)
 WHERE NOT EXISTS (
     SELECT 1
       FROM official_verification_metric_definition d
      WHERE d.metric_code = v.metric_code
        AND d.definition_version = v.definition_version
 );

INSERT INTO official_verification_metric_check_definition (
    metric_code, check_code, definition_version, check_label, expected_value, evidence_source, severity, remediation_owner
)
SELECT metric_code, metric_code || '_OFFICIAL_CONTRACT', definition_version,
       metric_name || ' official contract is registered',
       'active metric definition',
       'official_verification_metric_definition',
       'BLOCKING',
       'PQA_RUNTIME'
  FROM official_verification_metric_definition d
 WHERE d.definition_version = '2026.05.01-pqa12-p1'
   AND NOT EXISTS (
       SELECT 1
         FROM official_verification_metric_check_definition c
        WHERE c.metric_code = d.metric_code
          AND c.check_code = d.metric_code || '_OFFICIAL_CONTRACT'
...
SELECT metric_code, check_code, definition_version, check_label, expected_value, evidence_source, severity, remediation_owner
  FROM (
      VALUES
          ('EIR', 'EIR_MFA_STATE_PROMPT_MATCH', '2026.05.01-pqa12-p1',
           'MFA state in auth evidence matches the final user prompt', 'same key and value in authState and userPromptText',
           'sealedEvidence.authState.mfaVerified|userPromptText', 'BLOCKING', 'PROMPT_ASSEMBLER'),
          ('CCR', 'CCR_AUTH_CONTEXT_COMPLETE', '2026.05.01-pqa12-p1',
           'Authentication context contains required method, MFA, role, permission, and authorization facts', 'complete auth context',
           'sealedEvidence.authState', 'BLOCKING', 'AUTH_CONTEXT'),
          ('CCSR', 'CCSR_PROMPT_HASH_LEDGER_MATCH', '2026.05.01-pqa12-p1',
           'Declared prompt hash matches sealed prompt metadata', 'same prompt hash in package and metadata',
           'sealedEvidence.promptHash|promptExecutionMetadata.promptHash', 'BLOCKING', 'PROMPT_CAPTURE'),
          ('PFR', 'PFR_CONTRACT_VIOLATION_ZERO', '2026.05.01-pqa12-p1',
          ...
          ('RAP', 'RAP_DOC_AUTH_DECISION_PRESENT_1', '2026.05.01-pqa12-p1',
          ...
          ('RAP', 'RAP_DOC_INCLUDED_IS_AUTHORIZED_1', '2026.05.01-pqa12-p1',
           'Included document is authorized before it enters final prompt context', 'final included document authorized',
           'sealedEvidence.ragResults.relatedDocuments[1].finalContextIncluded|authorizationDecision', 'BLOCKING', 'RAG_AUTHORIZATION'),
          ('RAP', 'RAP_DOC_PERMISSION_SCOPE_PRESENT_1', '2026.05.01-pqa12-p1',
          ...
          ('RPI', 'RPI_RELATED_DOCUMENTS_NON_REGRESSIVE', '2026.05.01-pqa12-p1',
           'Related document count does not regress across verification rounds', 'non-regressive related document count',
           'sealedEvidence.repository.priorPackages.ragResults.relatedDocuments', 'BLOCKING', 'EVIDENCE_STORE'),
          ('BMA', 'BMA_MATURITY_NOT_OVERCLAIMED', '2026.05.01-pqa12-p1',
           'Baseline maturity is not overclaimed beyond observed data depth', 'maturity consistent with observation depth',
           'sealedEvidence.baselineSnapshot', 'BLOCKING', 'LEARNING_CONTEXT'),
          ('USNS', 'USNS_TIME_PROMPT_REFLECTS_SIGNAL', '2026.05.01-pqa12-p1',
           'Time novelty signal is reflected in the final user prompt', 'prompt mentions time novelty signal',
           'sealedEvidence.baselineSnapshot.noveltySignals.time|userPromptText', 'BLOCKING', 'PROMPT_ASSEMBLER'),
          ('BSR', 'BSR_PROMPT_EXPLAINS_BEHAVIOR', '2026.05.01-pqa12-p1',
           'Prompt explains behavior with baseline or observed work pattern context', 'behavior baseline explanation present',
           'sealedEvidence.canonicalContext|userPromptText', 'BLOCKING', 'PROMPT_ASSEMBLER'),
          ('PRE', 'PRE_RESOURCE_ID_BOUND_TO_ACTUAL', '2026.05.01-pqa12-p1',
           'Protectable resource template is bound to the actual request resource value', 'actual resource id bound to @Protectable template',
           'sealedEvidence.requestFacts.protectableResourceId|requestFacts.resourceId', 'BLOCKING', 'PROTECTABLE_RESOURCE')
  ) AS v(metric_code, check_code, definition_version, check_label, expected_value, evidence_source, severity, remediation_owner)
 WHERE NOT EXISTS (
     SELECT 1
       FROM official_verification_metric_check_definition c
      WHERE c.metric_code = v.metric_code
        AND c.check_code = v.check_code
        AND c.definition_version = v.definition_version
 );

DELETE FROM official_verification_metric_regression_expectation
 WHERE scenario_id LIKE 'pqa12-%';

DELETE FROM official_verification_metric_regression_package
 WHERE scenario_id LIKE 'pqa12-%';

INSERT INTO official_verification_metric_regression_package (
    scenario_id, scenario_name, scenario_type, target_metric_codes, fixture_source, expected_decision
)
SELECT scenario_id, scenario_name, scenario_type, target_metric_codes, fixture_source, expected_decision
  FROM (
      VALUES
          ('pqa12-clean-pass', 'All 12 prompt-quality metrics pass on representative sealed evidence', 'PASS_BASELINE',
           'EIR,CCR,CCSR,PFR,MTR,CoR,RAP,RPI,BMA,USNS,BSR,PRE',
           'io.contexa.contexacoreenterprise.verification.regression.OfficialPromptQualityMetricRegressionSuite', 'CERTIFIABLE'),
          ('pqa12-fail-eir', 'EIR fails when MFA state in auth evidence and prompt diverge', 'FAIL_SINGLE_METRIC',
           'EIR', 'io.contexa.contexacoreenterprise.verification.regression.OfficialPromptQualityMetricRegressionSuite', 'BLOCKED'),
          ('pqa12-fail-ccr', 'CCR fails when required auth context facts are missing', 'FAIL_SINGLE_METRIC',
           'CCR', 'io.contexa.contexacoreenterprise.verification.regression.OfficialPromptQualityMetricRegressionSuite', 'BLOCKED'),
          ('pqa12-fail-ccsr', 'CCSR fails when package prompt hash and metadata hash diverge', 'FAIL_SINGLE_METRIC',
           'CCSR', 'io.contexa.contexacoreenterprise.verification.regression.OfficialPromptQualityMetricRegressionSuite', 'BLOCKED'),
          ('pqa12-fail-pfr', 'PFR fails when the official prompt contract reports a violation', 'FAIL_SINGLE_METRIC',
           'PFR', 'io.contexa.contexacoreenterprise.verification.regression.OfficialPromptQualityMetricRegressionSuite', 'BLOCKED'),
          ('pqa12-fail-mtr', 'MTR fails when sealed evidence retention expiry is missing', 'FAIL_SINGLE_METRIC',
           'MTR', 'io.contexa.contexacoreenterprise.verification.regression.OfficialPromptQualityMetricRegressionSuite', 'BLOCKED'),
          ('pqa12-fail-cor', 'CoR fails when retrieved context violates purpose boundary', 'FAIL_SINGLE_METRIC',
           'CoR', 'io.contexa.contexacoreenterprise.verification.regression.OfficialPromptQualityMetricRegressionSuite', 'BLOCKED'),
          ('pqa12-fail-rap', 'RAP fails when included document authorization facts are missing', 'FAIL_SINGLE_METRIC',
           'RAP', 'io.contexa.contexacoreenterprise.verification.regression.OfficialPromptQualityMetricRegressionSuite', 'BLOCKED'),
          ('pqa12-fail-rpi', 'RPI fails when related document evidence regresses across rounds', 'FAIL_SINGLE_METRIC',
           'RPI', 'io.contexa.contexacoreenterprise.verification.regression.OfficialPromptQualityMetricRegressionSuite', 'BLOCKED'),
          ('pqa12-fail-bma', 'BMA fails when baseline maturity is overclaimed', 'FAIL_SINGLE_METRIC',
           'BMA', 'io.contexa.contexacoreenterprise.verification.regression.OfficialPromptQualityMetricRegressionSuite', 'BLOCKED'),
          ('pqa12-fail-usns', 'USNS fails when time novelty is not reflected in the prompt', 'FAIL_SINGLE_METRIC',
           'USNS', 'io.contexa.contexacoreenterprise.verification.regression.OfficialPromptQualityMetricRegressionSuite', 'BLOCKED'),
          ('pqa12-fail-bsr', 'BSR fails when behavior context is not explained in the prompt', 'FAIL_SINGLE_METRIC',
           'BSR', 'io.contexa.contexacoreenterprise.verification.regression.OfficialPromptQualityMetricRegressionSuite', 'BLOCKED'),
          ('pqa12-fail-pre', 'PRE fails when @Protectable template is not bound to the actual resource', 'FAIL_SINGLE_METRIC',
           'PRE', 'io.contexa.contexacoreenterprise.verification.regression.OfficialPromptQualityMetricRegressionSuite', 'BLOCKED')
  ) AS v(scenario_id, scenario_name, scenario_type, target_metric_codes, fixture_source, expected_decision);

INSERT INTO official_verification_metric_regression_expectation (
    scenario_id, metric_code, expected_state, expected_failed_check_codes
)
SELECT 'pqa12-clean-pass', d.metric_code, 'success', ''
  FROM official_verification_metric_definition d
 WHERE d.definition_version = '2026.05.01-pqa12-p1';
...
SELECT scenario_id, metric_code, expected_state, expected_failed_check_codes
  FROM (
      VALUES
          ('pqa12-fail-eir', 'EIR', 'threshold_failed', 'EIR_MFA_STATE_PROMPT_MATCH'),
          ('pqa12-fail-ccr', 'CCR', 'threshold_failed', 'CCR_AUTH_CONTEXT_COMPLETE'),
          ('pqa12-fail-ccsr', 'CCSR', 'threshold_failed', 'CCSR_PROMPT_HASH_LEDGER_MATCH'),
          ('pqa12-fail-pfr', 'PFR', 'threshold_failed', 'PFR_CONTRACT_VIOLATION_ZERO'),
          ('pqa12-fail-mtr', 'MTR', 'threshold_failed', 'MTR_EXPIRY_PRESENT'),
          ('pqa12-fail-cor', 'CoR', 'threshold_failed', 'COR_PURPOSE_MISMATCH_ZERO'),
          ('pqa12-fail-rap', 'RAP', 'threshold_failed', 'RAP_DOC_AUTH_DECISION_PRESENT_1,RAP_DOC_INCLUDED_IS_AUTHORIZED_1,RAP_DOC_PERMISSION_SCOPE_PRESENT_1'),
          ('pqa12-fail-rpi', 'RPI', 'threshold_failed', 'RPI_RELATED_DOCUMENTS_NON_REGRESSIVE'),
          ('pqa12-fail-bma', 'BMA', 'threshold_failed', 'BMA_MATURITY_NOT_OVERCLAIMED'),
          ('pqa12-fail-usns', 'USNS', 'threshold_failed', 'USNS_TIME_PROMPT_REFLECTS_SIGNAL'),
          ('pqa12-fail-bsr', 'BSR', 'threshold_failed', 'BSR_PROMPT_EXPLAINS_BEHAVIOR'),
          ('pqa12-fail-pre', 'PRE', 'threshold_failed', 'PRE_RESOURCE_ID_BOUND_TO_ACTUAL')
  ) AS v(scenario_id, metric_code, expected_state, expected_failed_check_codes);
...
UPDATE official_verification_metric_regression_package
   SET sample_kind = 'PASS_ALL',
       prompt_hash = 'sha256:pqa12-clean-pass',
       context_hash = 'sha256:ctx-pqa12-clean-pass',
...
UPDATE official_verification_metric_regression_package
   SET sample_kind = 'FAIL',
       prompt_hash = 'sha256:' || scenario_id,
       context_hash = 'sha256:ctx-' || scenario_id,
       resource_template_id = 'official.verification.normal.{resourceId}',
       actual_resource_id = 'resource-001',
...
UPDATE official_verification_metric_regression_expectation
...
DELETE FROM official_verification_metric_regression_expectation
 WHERE scenario_id LIKE 'pqa12-pass-%'
    OR scenario_id LIKE 'pqa12-edge-%';
...
INSERT INTO official_verification_metric_regression_package (
    scenario_id, scenario_name, scenario_type, target_metric_codes, fixture_source, expected_decision,
    sample_kind, prompt_hash, context_hash, resource_template_id, actual_resource_id, sample_package_json,
    expected_finding_snapshot, expected_remediation_group, expected_reverify_criterion,
    expected_audit_payload_json, customer_sentence_contract_json
)
SELECT scenario_id,
       metric_code || ' ' || sample_kind || ' fixed regression sample',
       scenario_type,
       metric_code,
       'io.contexa.contexacoreenterprise.verification.regression.OfficialPromptQualityMetricRegressionSuite',
       expected_decision,
       sample_kind,
       'sha256:' || scenario_id,
       'sha256:ctx-' || scenario_id,
  ...
  FROM (
      VALUES
          ('pqa12-pass-eir', 'EIR', 'PASS', 'PASS_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-pass-ccr', 'CCR', 'PASS', 'PASS_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-pass-ccsr', 'CCSR', 'PASS', 'PASS_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-pass-pfr', 'PFR', 'PASS', 'PASS_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-pass-mtr', 'MTR', 'PASS', 'PASS_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-pass-cor', 'CoR', 'PASS', 'PASS_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-pass-rap', 'RAP', 'PASS', 'PASS_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-pass-rpi', 'RPI', 'PASS', 'PASS_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-pass-bma', 'BMA', 'PASS', 'PASS_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-pass-usns', 'USNS', 'PASS', 'PASS_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-pass-bsr', 'BSR', 'PASS', 'PASS_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-pass-pre', 'PRE', 'PASS', 'PASS_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-edge-eir', 'EIR', 'EDGE', 'EDGE_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-edge-ccr', 'CCR', 'EDGE', 'EDGE_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-edge-ccsr', 'CCSR', 'EDGE', 'EDGE_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-edge-pfr', 'PFR', 'EDGE', 'EDGE_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-edge-mtr', 'MTR', 'EDGE', 'EDGE_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-edge-cor', 'CoR', 'EDGE', 'EDGE_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-edge-rap', 'RAP', 'EDGE', 'EDGE_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-edge-rpi', 'RPI', 'EDGE', 'EDGE_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-edge-bma', 'BMA', 'EDGE', 'EDGE_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-edge-usns', 'USNS', 'EDGE', 'EDGE_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-edge-bsr', 'BSR', 'EDGE', 'EDGE_SINGLE_METRIC', 'CERTIFIABLE'),
          ('pqa12-edge-pre', 'PRE', 'EDGE', 'EDGE_SINGLE_METRIC', 'CERTIFIABLE')
  ) AS v(scenario_id, metric_code, sample_kind, scenario_type, expected_decision);

INSERT INTO official_verification_metric_regression_expectation (
    scenario_id, metric_code, expected_state, expected_failed_check_codes,
    expected_score, expected_passed_check_count, expected_failed_check_count,
    expected_finding_snapshot, expected_remediation_group, expected_reverify_criterion, expected_audit_payload_json
)
SELECT scenario_id,
       metric_code,
       'success',
  ...
  FROM (
      VALUES
          ('pqa12-pass-eir', 'EIR', 'PASS'),
          ('pqa12-pass-ccr', 'CCR', 'PASS'),
          ('pqa12-pass-ccsr', 'CCSR', 'PASS'),
          ('pqa12-pass-pfr', 'PFR', 'PASS'),
          ('pqa12-pass-mtr', 'MTR', 'PASS'),
          ('pqa12-pass-cor', 'CoR', 'PASS'),
          ('pqa12-pass-rap', 'RAP', 'PASS'),
          ('pqa12-pass-rpi', 'RPI', 'PASS'),
          ('pqa12-pass-bma', 'BMA', 'PASS'),
          ('pqa12-pass-usns', 'USNS', 'PASS'),
          ('pqa12-pass-bsr', 'BSR', 'PASS'),
          ('pqa12-pass-pre', 'PRE', 'PASS'),
          ('pqa12-edge-eir', 'EIR', 'EDGE'),
          ('pqa12-edge-ccr', 'CCR', 'EDGE'),
          ('pqa12-edge-ccsr', 'CCSR', 'EDGE'),
          ('pqa12-edge-pfr', 'PFR', 'EDGE'),
          ('pqa12-edge-mtr', 'MTR', 'EDGE'),
          ('pqa12-edge-cor', 'CoR', 'EDGE'),
          ('pqa12-edge-rap', 'RAP', 'EDGE'),
          ('pqa12-edge-rpi', 'RPI', 'EDGE'),
          ('pqa12-edge-bma', 'BMA', 'EDGE'),
          ('pqa12-edge-usns', 'USNS', 'EDGE'),
          ('pqa12-edge-bsr', 'BSR', 'EDGE'),
          ('pqa12-edge-pre', 'PRE', 'EDGE')
  ) AS v(scenario_id, metric_code, sample_kind);
