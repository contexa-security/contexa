...
UPDATE official_verification_metric_regression_package
   SET sample_kind = 'PASS_ALL',
       ...
       expected_finding_snapshot = '12개 프롬프트 품질 지표 전체가 같은 봉인 증거와 같은 고객 문장 계약으로 통과해야 합니다.',
       expected_remediation_group = '실패 지표가 없으면 문제 해결 그룹을 만들지 않고 감사 근거로만 연결합니다.',
       expected_reverify_criterion = '같은 증거 구조로 다시 실행해도 12개 지표 전체가 통과해야 합니다.',
...
UPDATE official_verification_metric_regression_package
   SET sample_kind = 'FAIL',
       ...
       expected_finding_snapshot = target_metric_codes || ' 지표 실패 샘플은 어떤 증거 항목이 기준을 충족하지 못했는지 고객이 이해할 수 있는 문장으로 저장되어야 합니다.',
       expected_remediation_group = target_metric_codes || ' 지표 실패는 담당 컨텍스트 생산자와 프롬프트 조립 경로를 하나의 문제 해결 그룹으로 연결해야 합니다.',
       expected_reverify_criterion = target_metric_codes || ' 지표의 실패 체크가 다음 공식검사에서 통과 상태로 저장되어야 합니다.',
...
UPDATE official_verification_metric_regression_expectation
   SET expected_score = 90.00,
       ...
       expected_finding_snapshot = metric_code || ' 지표 실패 원인이 체크 단위로 저장되어야 합니다.',
       expected_remediation_group = metric_code || ' 지표 실패는 수정 책임 대상과 다음 조치로 묶여야 합니다.',
       expected_reverify_criterion = metric_code || ' 지표 실패 체크가 재검증에서 통과해야 합니다.',
...
SELECT scenario_id,
       ...
       metric_code || ' 지표 ' || sample_kind || ' 샘플은 실제 요청 resource-001과 @Protectable 템플릿 {resourceId}를 분리한 상태에서 같은 진단 문장을 보존해야 합니다.',
       metric_code || ' 지표의 문제 해결 그룹은 원인, 수정 책임 대상, 다음 조치를 고객 문장으로 저장해야 합니다.',
       metric_code || ' 지표는 같은 증거 조건으로 재검증했을 때 기대 상태와 고객 문장이 유지되어야 합니다.',
...
SELECT scenario_id,
       ...
       1,
       0,
       metric_code || ' 지표는 이 샘플에서 고객이 이해할 수 있는 통과 사유를 저장해야 합니다.',
       metric_code || ' 지표가 통과하면 별도 보강 조치 없이 감사 근거로 연결합니다.',
       metric_code || ' 지표는 같은 증거로 다시 실행해도 통과 상태와 문장이 유지되어야 합니다.',
...
binding_runtime AS (
    SELECT e.id,
      ...
      JOIN LATERAL (
          SELECT concat(source_fact.source_fact_key, ' 값은 ', v.value_preview) AS fact_text,
