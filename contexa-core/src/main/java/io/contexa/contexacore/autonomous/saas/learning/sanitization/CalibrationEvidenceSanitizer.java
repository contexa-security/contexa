...
/**
 * Generalizes calibration evidence into tenant-safe scenario facts.
 */
public class CalibrationEvidenceSanitizer extends AbstractLearningEvidenceSanitizer {
    ...
    protected String generalizeSensitiveFact(String fact) {
        ...
        if (normalized.contains("path") || normalized.contains("request") || normalized.contains("session")) {
            return "Scenario-context evidence was generalized to a tenant-safe calibration fact.";
        }
        if (normalized.contains("tenant") || normalized.contains("user") || normalized.contains("geo")) {
            return "Tenant-private calibration evidence was generalized to a scenario-level fact.";
        }
        return "Calibration evidence was generalized to a tenant-safe scenario fact.";
    }
