...
public class PolicyEvolutionEngine {
     ...
     */
    public void learnFromRejection(io.contexa.contexacore.autonomous.domain.PolicyDTO policy, String rejectionReason) {
     ...
     * PolicyChangeEventListener에서 호출
     */
    public void requestEvolution(io.contexa.contexacore.autonomous.domain.PolicyDTO policy, Map<String, Object> context) {
     ...
     * 유사 정책의 신뢰도 업데이트
     */
    private void updateSimilarPolicyConfidence(io.contexa.contexacore.autonomous.domain.PolicyDTO policy, double adjustment) {
        try {
            // 벡터 스토어에서 유사 정책 검색
            SearchRequest searchRequest = SearchRequest.builder()
     ...
     * 진화된 정책 생성
     */
    private void createEvolvedPolicy(io.contexa.contexacore.autonomous.domain.PolicyDTO originalPolicy,
                                    PolicyEvolutionProposal proposal) {
