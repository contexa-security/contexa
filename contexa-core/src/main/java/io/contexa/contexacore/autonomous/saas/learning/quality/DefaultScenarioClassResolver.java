...
/**
 * Resolves conservative scenario classes from correlated decision-quality observations.
 */
public class DefaultScenarioClassResolver implements ScenarioClassResolver {
    ...
    @Override
    public ScenarioClassResolution resolve(DecisionQualityObservation observation) {
        if (observation == null) {
    ...
    private ScenarioClassResolution resolveExplicitHint(DecisionQualityObservation observation) {
        ...
        if (Objects.equals(explicit, normalize(NEW_DEVICE_POST_MFA_SENSITIVE))) {
            return resolved(NEW_DEVICE_POST_MFA_SENSITIVE, "Scenario hint matched the fixed decision-quality class.");
        }
        if (Objects.equals(explicit, normalize(LOW_DIVERSITY_EXPORT_APPROACH))) {
            return resolved(LOW_DIVERSITY_EXPORT_APPROACH, "Scenario hint matched the fixed decision-quality class.");
        }
        if (Objects.equals(explicit, normalize(SESSION_PATH_SIMILARITY_BREAK))) {
            return resolved(SESSION_PATH_SIMILARITY_BREAK, "Scenario hint matched the fixed decision-quality class.");
        }
        return ScenarioClassResolution.unresolved();
    }

    private boolean isNewDevicePostMfaSensitive(DecisionQualityObservation observation) {
    ...
    private boolean isLowDiversityExportApproach(DecisionQualityObservation observation) {
    ...
    private boolean isSessionPathSimilarityBreak(DecisionQualityObservation observation) {
    ...
    private boolean lowDiversityHint(DecisionQualityObservation observation) {
    ...
    private boolean isSensitiveOrAdministrativeSurface(DecisionQualityObservation observation) {
    ...
    private Object signal(DecisionQualityObservation observation, String key) {
        Map<String, Object> signals = observation.scenarioSignals();
        return signals != null ? signals.get(key) : null;
    }

    private String signalText(DecisionQualityObservation observation, String key) {
    ...
    private boolean containsAnyEvidence(DecisionQualityObservation observation, String... candidates) {
