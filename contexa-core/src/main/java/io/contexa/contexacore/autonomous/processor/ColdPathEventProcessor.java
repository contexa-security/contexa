...
public class ColdPathEventProcessor implements IPathProcessor {
    ...
    public ProcessingResult processEvent(SecurityEvent event, double riskScore) {
        ...
        try {
            ...
            publishError(event.getUserId(), "AI analysis failed: " + e.getMessage());
            return ProcessingResult.failure(
                    ProcessingResult.ProcessingPath.COLD_PATH,
    ...
    private ThreatAnalysisResult performTieredAIAnalysis(SecurityEvent event, double riskScore) {
        ...
        try {
            ...
            publishError(userId, "Tiered AI analysis failed: " + e.getMessage());
            result.setFinalScore(null);
            result.setAction(ZeroTrustAction.CHALLENGE.name());
            result.setConfidence(null);
    ...
    private void publishLayer1Complete(String userId, String action,
                                       String reasoning, String mitre, Long elapsedMs) {
        llmAnalysisEventListener.onLayer1Complete(userId, action, reasoning, mitre, elapsedMs);
    }
    ...
    private void publishLayer2Complete(String userId, String action,
                                       String reasoning, String mitre, Long elapsedMs) {
        llmAnalysisEventListener.onLayer2Complete(userId, action, reasoning, mitre, elapsedMs);
    }
    ...
    private void publishError(String userId, String message) {
        llmAnalysisEventListener.onError(userId, message);
    }
