...
public class SignalInconsistencyDetector {
    ...
    private void saveSignalHistory(String userId, double[] signalVector) {
        ...
        // 최근 100개 저장 (공분산 계산 충분)
        redisTemplate.opsForList().leftPush(key, signalVector);
        redisTemplate.opsForList().trim(key, 0, signalHistorySize - 1);

        // TTL 7일
        redisTemplate.expire(key, Duration.ofDays(7));

        // 캐시 무효화 (다음 요청 시 재계산)
        redisTemplate.delete("signal:covariance:" + userId);
        redisTemplate.delete("signal:mean:" + userId);
    }
