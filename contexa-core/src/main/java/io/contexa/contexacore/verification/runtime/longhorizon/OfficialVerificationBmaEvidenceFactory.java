...
import io.contexa.contexacore.verification.runtime.longhorizon.OfficialVerificationBmaExecutionService.BmaEventItem;
import io.contexa.contexacore.verification.runtime.longhorizon.OfficialVerificationBmaExecutionService.BmaRunRecord;
...
final class OfficialVerificationBmaEvidenceFactory {

    private static final String SCENARIO_SELECTOR =
            OfficialVerificationPromptContractReplaySupport.EXTENDED_SCENARIO_SELECTOR;

    private final OfficialVerificationLongHorizonEvidenceValues values =
            new OfficialVerificationLongHorizonEvidenceValues();
    private final OfficialVerificationBmaCheckEvaluator checkEvaluator =
            new OfficialVerificationBmaCheckEvaluator();
    ...
    RoundSnapshot createRoundSnapshot(
        ...
        return new RoundSnapshot(
                ...
                relatedDocumentsCount, observationCount,
                baselineContextPresent, requestParityAligned, workProfileSummary
        );
    }

    List<BmaEventItem> toEventItems(RoundSnapshot round) {
    ...
    BmaRunRecord buildRunRecord(BmaRunInput state) {
    ...
    record BmaRunInput(
            String runId, int runOrdinal, String userId, int requestedRunCount, int horizonRounds,
            boolean rerun, boolean contaminationSeed, boolean baselineSeedRequested,
            OfficialVerificationExecutionRequest request, Instant startedAt, Instant completedAt,
            double score, int passedChecks, int totalChecks, Long processingTimeMs, boolean success,
            List<BmaCheckResult> checks, List<RoundSnapshot> rounds, List<BmaEventItem> aggregatedEvents,
    ...
    private Map<String, String> buildRequestFacts(
        ...
        facts.put("verificationUser", values.value(verificationUserId));
    ...
    private Map<String, String> buildEventFacts(List<RoundSnapshot> rounds) {
        ...
        facts.put("finalRequestPath", values.value(values.text(rounds.get(rounds.size() - 1).invocation(), "requestPath")));
        return facts;
    }

    private Map<String, String> buildPromptFacts(List<RoundSnapshot> rounds) {
        Map<String, String> facts = new LinkedHashMap<>();
        facts.put("initialRoundBaselineProvisional", Boolean.toString(checkEvaluator.provisionalBaseline(rounds.get(0))));
    ...
    private Map<String, String> buildAnalysisFacts(List<RoundSnapshot> rounds) {
        ...
        facts.put("observationCountNonDecreasing", Boolean.toString(checkEvaluator.observationCountsNonDecreasing(rounds)));
        facts.put("initialRoundBaselineProvisional", Boolean.toString(checkEvaluator.provisionalBaseline(rounds.get(0))));
        facts.put("finalBaselineContextPresent", Boolean.toString(rounds.get(rounds.size() - 1).baselineContextPresent()));
    ...
    private Map<String, Object> buildRawEvidence(
        ...
        summary.put("relatedDocumentsNonDecreasing", checkEvaluator.relatedDocumentsNonDecreasing(rounds));
        summary.put("observationCountNonDecreasing", checkEvaluator.observationCountsNonDecreasing(rounds));
        ...
        evidence.put("requestedPreset", Map.of(
                "verificationUser", values.value(verificationUserId),
    ...
    private Map<String, Object> buildRoundEvidence(RoundSnapshot round) {
        ...
        evidence.put("requestPath", values.text(round.invocation(), "requestPath"));
