 ...
 * 
 * 통합된 승인 메커니즘을 제공하여 단순하고 일관된 승인 처리를 보장합니다.
 */
public interface ApprovalService {
     ...
     * @param requestDetails 승인 요청에 대한 상세 내용 (e.g., 실행할 액션, 파라미터)
     ...
     * @return 승인 상태 (PENDING, APPROVED, REJECTED)
     */
    io.contexa.contexacore.domain.ApprovalRequest.ApprovalStatus getApprovalStatus(String approvalId);
    ...
    default ApprovalRequest saveApprovalRequest(ApprovalRequest request) {
        // 기본 구현: 기존 requestApproval 메서드 호출
        // 실제 구현은 DefaultApprovalService에서 오버라이드
    ...
    default Mono<Boolean> waitForApproval(String requestId) {
        ...
        // 실제 구현은 DefaultApprovalService에서 오버라이드
        return Mono.defer(() -> {
            io.contexa.contexacore.domain.ApprovalRequest.ApprovalStatus status = getApprovalStatus(requestId);
            return Mono.just(status == io.contexa.contexacore.domain.ApprovalRequest.ApprovalStatus.APPROVED);
        });
    }
    ...
    default void processApproval(String approvalId, boolean approved, String reason) {
        // 기본 구현: handleApprovalResponse 호출
     ...
     */
    default java.util.List<ApprovalRequest> getPendingApprovals() {
        // 기본 구현: 빈 리스트 반환
        // 실제 구현은 DefaultApprovalService에서 오버라이드
