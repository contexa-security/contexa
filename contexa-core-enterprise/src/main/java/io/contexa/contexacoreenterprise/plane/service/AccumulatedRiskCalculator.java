...
public class AccumulatedRiskCalculator {
    ...
    /**
     * 위험 레벨 임계값 (application.yml에서 주입)
     */
    @Value("${risk.level.critical:0.85}")
    private double criticalRiskThreshold;

    @Value("${risk.level.high:0.7}")
    private double highRiskThreshold;

    @Value("${risk.level.medium:0.5}")
    private double mediumRiskThreshold;

    @Value("${risk.level.low:0.3}")
    ...
    private RiskLevel determineRiskLevel(double accumulatedRisk) {
        if (accumulatedRisk >= criticalRiskThreshold) {
            return RiskLevel.CRITICAL;
        } else if (accumulatedRisk >= highRiskThreshold) {
            return RiskLevel.HIGH;
        } else if (accumulatedRisk >= mediumRiskThreshold) {
            return RiskLevel.MEDIUM;
        } else if (accumulatedRisk >= lowRiskThreshold) {
