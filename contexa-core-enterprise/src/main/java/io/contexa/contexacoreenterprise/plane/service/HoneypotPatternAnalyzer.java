...
public class HoneypotPatternAnalyzer {
    ...
    public HoneypotAnalysisResult analyzeSensitiveAccess(SecurityEvent event) {
        ...
        // 2. 접근 기록 조회 (한 번만 수행)
        List<AccessRecord> accessHistory = getAccessHistory(userId);

        // 3. 접근 빈도 분석
        FrequencyAnalysisResult frequencyResult = analyzeAccessFrequency(accessHistory);

        // 4. 민감 리소스 다양성 분석
        DiversityAnalysisResult diversityResult = analyzeSensitiveDiversity(accessHistory);

        // 5. 순차 접근 패턴 분석
        SequentialPatternResult sequentialResult = analyzeSequentialPattern(accessHistory);
    ...
    private List<AccessRecord> getAccessHistory(String userId) {
        String key = buildAccessKey(userId);
        List<AccessRecord> history = (List<AccessRecord>) (List<?>) redisTemplate.opsForList().range(key, 0, -1);
     ...
     * 접근 빈도 분석
     */
    private FrequencyAnalysisResult analyzeAccessFrequency(List<AccessRecord> allRecords) {
     ...
     * 민감 리소스 다양성 분석
     */
    private DiversityAnalysisResult analyzeSensitiveDiversity(List<AccessRecord> allRecords) {
        if (allRecords.isEmpty()) {
     ...
     */
    private SequentialPatternResult analyzeSequentialPattern(List<AccessRecord> allRecords) {
